/*******************************************************************************
* File Name: Cyarlobot_sys_clock.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: Cyarlobot_sys_clock_Start
********************************************************************************
*
* Summary:
...
void Cyarlobot_sys_clock_Start(void) 
{
    ...
    Cyarlobot_sys_clock_CLKEN |= Cyarlobot_sys_clock_CLKEN_MASK;
	Cyarlobot_sys_clock_CLKSTBY |= Cyarlobot_sys_clock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Cyarlobot_sys_clock_Stop
********************************************************************************
*
* Summary:
...
void Cyarlobot_sys_clock_Stop(void) 
{
    ...
    Cyarlobot_sys_clock_CLKEN &= (uint8)(~Cyarlobot_sys_clock_CLKEN_MASK);
	Cyarlobot_sys_clock_CLKSTBY &= (uint8)(~Cyarlobot_sys_clock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Cyarlobot_sys_clock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Cyarlobot_sys_clock_CLKEN & Cyarlobot_sys_clock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Cyarlobot_sys_clock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Cyarlobot_sys_clock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Cyarlobot_sys_clock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Cyarlobot_sys_clock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Cyarlobot_sys_clock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Cyarlobot_sys_clock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Cyarlobot_sys_clock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Cyarlobot_sys_clock_GetSourceRegister();
    uint16 oldDivider = Cyarlobot_sys_clock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Cyarlobot_sys_clock_CLKEN & Cyarlobot_sys_clock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Cyarlobot_sys_clock_DIV_PTR, clkDivider);
                Cyarlobot_sys_clock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Cyarlobot_sys_clock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Cyarlobot_sys_clock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Cyarlobot_sys_clock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(Cyarlobot_sys_clock_DIV_PTR, clkDivider);
				Cyarlobot_sys_clock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: Cyarlobot_sys_clock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Cyarlobot_sys_clock_DIV_PTR);
}


/*******************************************************************************
* Function Name: Cyarlobot_sys_clock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Cyarlobot_sys_clock_MOD_SRC |= modeBitMask & (uint8)Cyarlobot_sys_clock_MODE_MASK;
}


/*******************************************************************************
* Function Name: Cyarlobot_sys_clock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Cyarlobot_sys_clock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Cyarlobot_sys_clock_MODE_MASK));
}


/*******************************************************************************
* Function Name: Cyarlobot_sys_clock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Cyarlobot_sys_clock_MOD_SRC & (uint8)(Cyarlobot_sys_clock_MODE_MASK);
}


/*******************************************************************************
* Function Name: Cyarlobot_sys_clock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Cyarlobot_sys_clock_GetDividerRegister();
    uint8 oldSrc = Cyarlobot_sys_clock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Cyarlobot_sys_clock_MOD_SRC |= CYCLK_SSS;
        Cyarlobot_sys_clock_MOD_SRC =
            (Cyarlobot_sys_clock_MOD_SRC & (uint8)(~Cyarlobot_sys_clock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Cyarlobot_sys_clock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Cyarlobot_sys_clock_MOD_SRC & Cyarlobot_sys_clock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Cyarlobot_sys_clock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Cyarlobot_sys_clock_PHASE = clkPhase & Cyarlobot_sys_clock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Cyarlobot_sys_clock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Cyarlobot_sys_clock_PHASE & Cyarlobot_sys_clock_PHASE_MASK;
}
