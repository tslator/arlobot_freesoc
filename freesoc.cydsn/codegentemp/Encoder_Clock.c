...
void Encoder_Clock_Start(void) 
{
    ...
    Encoder_Clock_CLKEN |= Encoder_Clock_CLKEN_MASK;
	Encoder_Clock_CLKSTBY |= Encoder_Clock_CLKSTBY_MASK;
}
...
void Encoder_Clock_Stop(void) 
{
    ...
    Encoder_Clock_CLKEN &= (uint8)(~Encoder_Clock_CLKEN_MASK);
	Encoder_Clock_CLKSTBY &= (uint8)(~Encoder_Clock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Encoder_Clock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Encoder_Clock_CLKEN & Encoder_Clock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Encoder_Clock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Encoder_Clock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Encoder_Clock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Encoder_Clock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Encoder_Clock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Encoder_Clock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Encoder_Clock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Encoder_Clock_GetSourceRegister();
    uint16 oldDivider = Encoder_Clock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Encoder_Clock_CLKEN & Encoder_Clock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Encoder_Clock_DIV_PTR, clkDivider);
                Encoder_Clock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Encoder_Clock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Encoder_Clock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Encoder_Clock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
{
    return CY_GET_REG16(Encoder_Clock_DIV_PTR);
}


/*******************************************************************************
* Function Name: Encoder_Clock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Encoder_Clock_MOD_SRC |= modeBitMask & (uint8)Encoder_Clock_MODE_MASK;
}


/*******************************************************************************
* Function Name: Encoder_Clock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Encoder_Clock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Encoder_Clock_MODE_MASK));
}


/*******************************************************************************
* Function Name: Encoder_Clock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Encoder_Clock_MOD_SRC & (uint8)(Encoder_Clock_MODE_MASK);
}


/*******************************************************************************
* Function Name: Encoder_Clock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Encoder_Clock_GetDividerRegister();
    uint8 oldSrc = Encoder_Clock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Encoder_Clock_MOD_SRC |= CYCLK_SSS;
        Encoder_Clock_MOD_SRC =
            (Encoder_Clock_MOD_SRC & (uint8)(~Encoder_Clock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Encoder_Clock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Encoder_Clock_MOD_SRC & Encoder_Clock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Encoder_Clock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Encoder_Clock_PHASE = clkPhase & Encoder_Clock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Encoder_Clock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Encoder_Clock_PHASE & Encoder_Clock_PHASE_MASK;
}
