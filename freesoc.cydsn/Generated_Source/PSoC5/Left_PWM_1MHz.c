...
/*******************************************************************************
* Function Name: Left_PWM_1MHz_Start
********************************************************************************
*
* Summary:
...
void Left_PWM_1MHz_Start(void) 
{
    ...
    Left_PWM_1MHz_CLKEN |= Left_PWM_1MHz_CLKEN_MASK;
	Left_PWM_1MHz_CLKSTBY |= Left_PWM_1MHz_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Left_PWM_1MHz_Stop
********************************************************************************
*
* Summary:
...
void Left_PWM_1MHz_Stop(void) 
{
    ...
    Left_PWM_1MHz_CLKEN &= (uint8)(~Left_PWM_1MHz_CLKEN_MASK);
	Left_PWM_1MHz_CLKSTBY &= (uint8)(~Left_PWM_1MHz_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Left_PWM_1MHz_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Left_PWM_1MHz_CLKEN & Left_PWM_1MHz_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Left_PWM_1MHz_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Left_PWM_1MHz_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Left_PWM_1MHz__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Left_PWM_1MHz_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Left_PWM_1MHz_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Left_PWM_1MHz_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Left_PWM_1MHz_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Left_PWM_1MHz_GetSourceRegister();
    uint16 oldDivider = Left_PWM_1MHz_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Left_PWM_1MHz_CLKEN & Left_PWM_1MHz_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Left_PWM_1MHz_DIV_PTR, clkDivider);
                Left_PWM_1MHz_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Left_PWM_1MHz_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Left_PWM_1MHz_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Left_PWM_1MHz_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Left_PWM_1MHz_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Left_PWM_1MHz_DIV_PTR);
}


/*******************************************************************************
* Function Name: Left_PWM_1MHz_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Left_PWM_1MHz_MOD_SRC |= modeBitMask & (uint8)Left_PWM_1MHz_MODE_MASK;
}


/*******************************************************************************
* Function Name: Left_PWM_1MHz_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Left_PWM_1MHz_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Left_PWM_1MHz_MODE_MASK));
}


/*******************************************************************************
* Function Name: Left_PWM_1MHz_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Left_PWM_1MHz_MOD_SRC & (uint8)(Left_PWM_1MHz_MODE_MASK);
}


/*******************************************************************************
* Function Name: Left_PWM_1MHz_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Left_PWM_1MHz_GetDividerRegister();
    uint8 oldSrc = Left_PWM_1MHz_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Left_PWM_1MHz_MOD_SRC |= CYCLK_SSS;
        Left_PWM_1MHz_MOD_SRC =
            (Left_PWM_1MHz_MOD_SRC & (uint8)(~Left_PWM_1MHz_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Left_PWM_1MHz_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Left_PWM_1MHz_MOD_SRC & Left_PWM_1MHz_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Left_PWM_1MHz_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Left_PWM_1MHz_PHASE = clkPhase & Left_PWM_1MHz_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Left_PWM_1MHz_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Left_PWM_1MHz_PHASE & Left_PWM_1MHz_PHASE_MASK;
}
