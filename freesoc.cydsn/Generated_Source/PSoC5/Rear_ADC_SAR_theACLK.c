/*******************************************************************************
* File Name: Rear_ADC_SAR_theACLK.c
* Version 2.10
*
*  Description:
...
/*******************************************************************************
* Function Name: Rear_ADC_SAR_theACLK_Start
********************************************************************************
*
* Summary:
...
void Rear_ADC_SAR_theACLK_Start(void) 
{
    ...
    Rear_ADC_SAR_theACLK_CLKEN |= Rear_ADC_SAR_theACLK_CLKEN_MASK;
	Rear_ADC_SAR_theACLK_CLKSTBY |= Rear_ADC_SAR_theACLK_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Rear_ADC_SAR_theACLK_Stop
********************************************************************************
*
* Summary:
...
void Rear_ADC_SAR_theACLK_Stop(void) 
{
    ...
    Rear_ADC_SAR_theACLK_CLKEN &= (uint8)(~Rear_ADC_SAR_theACLK_CLKEN_MASK);
	Rear_ADC_SAR_theACLK_CLKSTBY &= (uint8)(~Rear_ADC_SAR_theACLK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Rear_ADC_SAR_theACLK_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Rear_ADC_SAR_theACLK_CLKEN & Rear_ADC_SAR_theACLK_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Rear_ADC_SAR_theACLK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Rear_ADC_SAR_theACLK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Rear_ADC_SAR_theACLK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Rear_ADC_SAR_theACLK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Rear_ADC_SAR_theACLK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Rear_ADC_SAR_theACLK_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Rear_ADC_SAR_theACLK_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Rear_ADC_SAR_theACLK_GetSourceRegister();
    uint16 oldDivider = Rear_ADC_SAR_theACLK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Rear_ADC_SAR_theACLK_CLKEN & Rear_ADC_SAR_theACLK_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Rear_ADC_SAR_theACLK_DIV_PTR, clkDivider);
                Rear_ADC_SAR_theACLK_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Rear_ADC_SAR_theACLK_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Rear_ADC_SAR_theACLK_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Rear_ADC_SAR_theACLK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(Rear_ADC_SAR_theACLK_DIV_PTR, clkDivider);
				Rear_ADC_SAR_theACLK_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: Rear_ADC_SAR_theACLK_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Rear_ADC_SAR_theACLK_DIV_PTR);
}


/*******************************************************************************
* Function Name: Rear_ADC_SAR_theACLK_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Rear_ADC_SAR_theACLK_MOD_SRC |= modeBitMask & (uint8)Rear_ADC_SAR_theACLK_MODE_MASK;
}


/*******************************************************************************
* Function Name: Rear_ADC_SAR_theACLK_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Rear_ADC_SAR_theACLK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Rear_ADC_SAR_theACLK_MODE_MASK));
}


/*******************************************************************************
* Function Name: Rear_ADC_SAR_theACLK_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Rear_ADC_SAR_theACLK_MOD_SRC & (uint8)(Rear_ADC_SAR_theACLK_MODE_MASK);
}


/*******************************************************************************
* Function Name: Rear_ADC_SAR_theACLK_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Rear_ADC_SAR_theACLK_GetDividerRegister();
    uint8 oldSrc = Rear_ADC_SAR_theACLK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Rear_ADC_SAR_theACLK_MOD_SRC |= CYCLK_SSS;
        Rear_ADC_SAR_theACLK_MOD_SRC =
            (Rear_ADC_SAR_theACLK_MOD_SRC & (uint8)(~Rear_ADC_SAR_theACLK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Rear_ADC_SAR_theACLK_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Rear_ADC_SAR_theACLK_MOD_SRC & Rear_ADC_SAR_theACLK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Rear_ADC_SAR_theACLK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Rear_ADC_SAR_theACLK_PHASE = clkPhase & Rear_ADC_SAR_theACLK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Rear_ADC_SAR_theACLK_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Rear_ADC_SAR_theACLK_PHASE & Rear_ADC_SAR_theACLK_PHASE_MASK;
}
