/*******************************************************************************
* File Name: Front_ADC_SAR_theACLK.c
* Version 2.10
*
*  Description:
...
/*******************************************************************************
* Function Name: Front_ADC_SAR_theACLK_Start
********************************************************************************
*
* Summary:
...
void Front_ADC_SAR_theACLK_Start(void) 
{
    ...
    Front_ADC_SAR_theACLK_CLKEN |= Front_ADC_SAR_theACLK_CLKEN_MASK;
	Front_ADC_SAR_theACLK_CLKSTBY |= Front_ADC_SAR_theACLK_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Front_ADC_SAR_theACLK_Stop
********************************************************************************
*
* Summary:
...
void Front_ADC_SAR_theACLK_Stop(void) 
{
    ...
    Front_ADC_SAR_theACLK_CLKEN &= (uint8)(~Front_ADC_SAR_theACLK_CLKEN_MASK);
	Front_ADC_SAR_theACLK_CLKSTBY &= (uint8)(~Front_ADC_SAR_theACLK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Front_ADC_SAR_theACLK_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Front_ADC_SAR_theACLK_CLKEN & Front_ADC_SAR_theACLK_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Front_ADC_SAR_theACLK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Front_ADC_SAR_theACLK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Front_ADC_SAR_theACLK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Front_ADC_SAR_theACLK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Front_ADC_SAR_theACLK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Front_ADC_SAR_theACLK_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Front_ADC_SAR_theACLK_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Front_ADC_SAR_theACLK_GetSourceRegister();
    uint16 oldDivider = Front_ADC_SAR_theACLK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Front_ADC_SAR_theACLK_CLKEN & Front_ADC_SAR_theACLK_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Front_ADC_SAR_theACLK_DIV_PTR, clkDivider);
                Front_ADC_SAR_theACLK_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Front_ADC_SAR_theACLK_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Front_ADC_SAR_theACLK_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Front_ADC_SAR_theACLK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(Front_ADC_SAR_theACLK_DIV_PTR, clkDivider);
				Front_ADC_SAR_theACLK_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: Front_ADC_SAR_theACLK_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Front_ADC_SAR_theACLK_DIV_PTR);
}


/*******************************************************************************
* Function Name: Front_ADC_SAR_theACLK_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Front_ADC_SAR_theACLK_MOD_SRC |= modeBitMask & (uint8)Front_ADC_SAR_theACLK_MODE_MASK;
}


/*******************************************************************************
* Function Name: Front_ADC_SAR_theACLK_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Front_ADC_SAR_theACLK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Front_ADC_SAR_theACLK_MODE_MASK));
}


/*******************************************************************************
* Function Name: Front_ADC_SAR_theACLK_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Front_ADC_SAR_theACLK_MOD_SRC & (uint8)(Front_ADC_SAR_theACLK_MODE_MASK);
}


/*******************************************************************************
* Function Name: Front_ADC_SAR_theACLK_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Front_ADC_SAR_theACLK_GetDividerRegister();
    uint8 oldSrc = Front_ADC_SAR_theACLK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Front_ADC_SAR_theACLK_MOD_SRC |= CYCLK_SSS;
        Front_ADC_SAR_theACLK_MOD_SRC =
            (Front_ADC_SAR_theACLK_MOD_SRC & (uint8)(~Front_ADC_SAR_theACLK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Front_ADC_SAR_theACLK_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Front_ADC_SAR_theACLK_MOD_SRC & Front_ADC_SAR_theACLK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Front_ADC_SAR_theACLK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Front_ADC_SAR_theACLK_PHASE = clkPhase & Front_ADC_SAR_theACLK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Front_ADC_SAR_theACLK_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Front_ADC_SAR_theACLK_PHASE & Front_ADC_SAR_theACLK_PHASE_MASK;
}
