...
/*******************************************************************************
* Function Name: Right_PWM_1MHz_Start
********************************************************************************
*
* Summary:
...
void Right_PWM_1MHz_Start(void) 
{
    ...
    Right_PWM_1MHz_CLKEN |= Right_PWM_1MHz_CLKEN_MASK;
	Right_PWM_1MHz_CLKSTBY |= Right_PWM_1MHz_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Right_PWM_1MHz_Stop
********************************************************************************
*
* Summary:
...
void Right_PWM_1MHz_Stop(void) 
{
    ...
    Right_PWM_1MHz_CLKEN &= (uint8)(~Right_PWM_1MHz_CLKEN_MASK);
	Right_PWM_1MHz_CLKSTBY &= (uint8)(~Right_PWM_1MHz_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Right_PWM_1MHz_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Right_PWM_1MHz_CLKEN & Right_PWM_1MHz_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Right_PWM_1MHz_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Right_PWM_1MHz_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Right_PWM_1MHz__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Right_PWM_1MHz_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Right_PWM_1MHz_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Right_PWM_1MHz_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Right_PWM_1MHz_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Right_PWM_1MHz_GetSourceRegister();
    uint16 oldDivider = Right_PWM_1MHz_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Right_PWM_1MHz_CLKEN & Right_PWM_1MHz_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Right_PWM_1MHz_DIV_PTR, clkDivider);
                Right_PWM_1MHz_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Right_PWM_1MHz_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Right_PWM_1MHz_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Right_PWM_1MHz_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Right_PWM_1MHz_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Right_PWM_1MHz_DIV_PTR);
}


/*******************************************************************************
* Function Name: Right_PWM_1MHz_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Right_PWM_1MHz_MOD_SRC |= modeBitMask & (uint8)Right_PWM_1MHz_MODE_MASK;
}


/*******************************************************************************
* Function Name: Right_PWM_1MHz_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Right_PWM_1MHz_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Right_PWM_1MHz_MODE_MASK));
}


/*******************************************************************************
* Function Name: Right_PWM_1MHz_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Right_PWM_1MHz_MOD_SRC & (uint8)(Right_PWM_1MHz_MODE_MASK);
}


/*******************************************************************************
* Function Name: Right_PWM_1MHz_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Right_PWM_1MHz_GetDividerRegister();
    uint8 oldSrc = Right_PWM_1MHz_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Right_PWM_1MHz_MOD_SRC |= CYCLK_SSS;
        Right_PWM_1MHz_MOD_SRC =
            (Right_PWM_1MHz_MOD_SRC & (uint8)(~Right_PWM_1MHz_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Right_PWM_1MHz_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Right_PWM_1MHz_MOD_SRC & Right_PWM_1MHz_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Right_PWM_1MHz_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Right_PWM_1MHz_PHASE = clkPhase & Right_PWM_1MHz_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Right_PWM_1MHz_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Right_PWM_1MHz_PHASE & Right_PWM_1MHz_PHASE_MASK;
}
